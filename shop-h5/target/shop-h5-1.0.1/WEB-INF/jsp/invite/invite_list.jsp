...
<section class="main">
	<div class="message">
        <ul class="meslist">
        	...
        	<%-- <li>${level }<li> --%>
        	<li>当前用户及其下级未审核好友（${UninvitedPerson } 人）<li>
        </ul>
    </div>
    <ul  class="person">
	    	...
	    	<!-- （addFriend字段0是，1否。字段为null，则是未审核） -->
		        <li class="arrow_parent">
		            ...
		            <a href="/invite/inviteList?currentUserId=${user.userId}" />
		                ...
		                <%-- <span>
		                	<a style="color:green;" href="/invite/inviteList?currentUserId=${user.userId}">${user.userName }</a>
		                </span> --%>
		                <span>
		                	<c:if test="${user.userType == '1'}"><!-- 顶级代理商 -->
		                	...
		                	<c:if test="${user.userType == '0'}"><!-- 普通用户 -->
		                		<c:if test="${user.addFriend == null || user.addFriend == ''}">
			                		<span>未审核</span>
		                		</c:if>
		                		<c:if test="${user.addFriend == '0'}">
