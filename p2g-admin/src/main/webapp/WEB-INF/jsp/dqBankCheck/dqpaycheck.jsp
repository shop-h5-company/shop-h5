    ...
    <form action="/dqBankCheck/addSubmit" class="pageForm" data-toggle="validate">
        ...
        <table class="table table-condensed table-hover">
            ...
            <tr>
                ...
                <td>
                    <label class="control-label x95">审核状态：</label>
                    <c:if test="${dqBankCheck.checkStatus=='not_do'}">
                        未审核
                    </c:if>
                    <c:if test="${dqBankCheck.checkStatus=='pass'}">
                        通过
                    </c:if>
                    <c:if test="${dqBankCheck.checkStatus=='back'}">
                        退回
                    </c:if>
                </td>
            ...
            <tr>
                ...
                <td>
                    <label for="payStatus"  class="control-label x95">付款状态：</label>
                    <select name="payStatus" id="payStatus" data-toggle="selectpicker" data-width="150">
                        <option value="not_do" <c:if test="${dqBankCheck.payStatus=='not_do' }">selected</c:if>>初审通过 </option>
                        <option value="success" <c:if test="${dqBankCheck.payStatus=='success' }">selected</c:if>>成功</option>
                        <option value="back" <c:if test="${dqBankCheck.payStatus=='back' }">selected</c:if>>退回</option>
                        <option value="fail" <c:if test="${dqBankCheck.payStatus=='fail' }">selected</c:if>>失败</option>
                    </select>
                </td>
                <td>
